import {configureClientAuthentication, configureClientCredentials401Retry, OAuthClient} from "./OAuthClient";
...
test('configureClientCredentials401Retry', async () => {
    ...
    const mock = new AxiosMockAdapter(httpClient, { onNoMatch: "throwException" });
    ...
    await expect(httpClient.request({
        method: 'GET',
        url: '/public',
    }).then(r => r.data)).resolves.toBe(42);
