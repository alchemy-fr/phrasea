            ...
            ->children()
                ...
                ->arrayNode('pusher')
                    ...
                    ->children()
                        ...
                        ->end()
                        ->scalarNode('port')
                            ->info('Port of the Soketi server, server side (defaults to 443)')
                            ->defaultValue('%env(SOKETI_PORT)%')
                        ->end()
                        ->scalarNode('scheme')
                            ->info('Scheme used to reach the Soketi server, server side (defaults to https)')
                            ->defaultValue('%env(SOKETI_SCHEME)%')
                        ->end()
                        ->scalarNode('channel_prefix')
                            ->info('Prefix of the per-user private channel a user may authorize (suffixed with the user identifier)')
                            ->cannotBeEmpty()
