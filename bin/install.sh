...
docker-compose ${CONF} exec -T uploader_api_php /bin/sh -c \
...
docker-compose ${CONF} exec -T expose_api_php /bin/sh -c \
    "bin/console doctrine:database:create --if-not-exists; bin/console doctrine:schema:update -f"

docker-compose ${CONF} exec -T auth_api_php /bin/sh -c \
    "bin/console doctrine:database:create --if-not-exists; bin/console doctrine:schema:update -f"
docker-compose ${CONF} exec -T auth_api_php /bin/sh -c \
