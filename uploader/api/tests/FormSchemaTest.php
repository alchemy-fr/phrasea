...
class FormSchemaTest extends AbstractUploaderTestCase
{
    ...
    {
        $response = $this->request(OAuthClientTestMock::getJwtFor(OAuthClientTestMock::ADMIN_UID), 'GET', '/form-schemas');
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertEquals('[]', $response->getContent());

        $response = $this->request(OAuthClientTestMock::getJwtFor(OAuthClientTestMock::ADMIN_UID), 'POST', '/form-schemas', [
    ...
    {
        $response = $this->request(OAuthClientTestMock::getJwtFor(OAuthClientTestMock::USER_UID), 'POST', '/form-schemas', [
            'data' => [
                'foo' => 'bar',
            ],
        ]);
