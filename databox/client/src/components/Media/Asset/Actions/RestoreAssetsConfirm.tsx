...
export default function RestoreAssetsConfirm({
    ...
    const restorableAssets = assets.filter(
        a => !a.referenceCollection?.deleted
    );
    ...
    const collections: Collection[] = (() => {
        ...
        assets.forEach(a => {
            if (a.referenceCollection?.deleted && a.referenceCollection.id) {
            ...
            {collections.length > 0 ? (
                <Box sx={{mt: 2}}>
                    <Alert severity={'warning'}>
                        {t(
                            'asset.restore.deleted_collections_warning',
                            'Note: Some assets cannot be restored because their reference collections have been deleted:'
                        )}
                    </Alert>
                    <List>
                        {collections.map(c => (
                            <ListItem key={c!.id}>
                                <ListItemText
                                    primary={<CollectionChip collection={c} />}
                                />
                                <ListItemSecondaryAction>
                                    <Button
