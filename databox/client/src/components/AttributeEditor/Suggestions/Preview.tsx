...
export default function Preview<T>({
    ...
    const [asset, setAsset] = React.useState<Asset | undefined>(subSelection[0]);
    const containerRef = React.useRef<HTMLDivElement | null>(null);
    const size = useElementResize(containerRef.current);
    const finalWidth = size ? size.width : defaultPanelWidth;
    const index: number = asset ? subSelection.findIndex(i => i.id === asset.id) : -1;

    const goTo = React.useCallback((offset: number) => {
        setAsset(p => {
            if (!p) {
            ...
            return subSelection[(index + subSelection.length + offset) % subSelection.length];
        })
    }, [subSelection]);
    ...
    return <div ref={containerRef}>
        {asset ? <>
            ...
            >
                ...
                >{asset.resolvedTitle}</Typography>
            </Box>
            {index >= 0 ? <Box sx={theme => ({
                p: 1,
                position: 'absolute',
