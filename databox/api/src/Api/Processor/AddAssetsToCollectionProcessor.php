...
class AddAssetsToCollectionProcessor implements ProcessorInterface
{
    ...
    {
        $collection = $this->resolveDestination($data->destination);
    ...
    private function resolveDestination(string $iri): Collection
    {
        ...
        if ($destination instanceof Asset) {
            $storyCollection = $destination->getStoryCollection();
            if (!$storyCollection instanceof Collection) {
                throw new BadRequestHttpException('Destination asset is not a story');
            }
