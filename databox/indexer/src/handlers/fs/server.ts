...
export const fsAssetServerFactory: AssetServerFactory<FsConfig> = function (
    location,
    logger
) {
    const {watchDir, dirPrefix} = getDirConfig(location.options);

    return async (path, res) => {
        const storagePath = dirPrefix
            ? watchDir + path.substring(dirPrefix.length)
            : path;
