                ...
                await runWithConcurrency(
                    pending.map(p => async () => {
                        try {
                            ...
                            uploadStore.setError(p.id, e?.message ?? String(e));
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                    ...
                    >
                        <TabsList>
                            <TabsTrigger value="files">
                                <UploadCloudIcon /> {t('upload.files', 'Files')}
                                {files.length > 0 ? (
                                    <Badge variant="muted">
                                        {files.length}
                                    </Badge>
                ...
                <div className="space-y-4">
                            ...
                            onApply={tpl => {
                                ...
                                if (tpl.attributes) {
                                    assetEditor.setAttributes(prev => {
                                        const next = {...prev};
                                        (tpl.attributes as any[]).forEach(a => {
                                            const defId =
                                                a.definition?.id ??
                                                a.definitionId;
                                            if (
                                                !defId ||
                                                !assetEditor.definitions[defId]
                                            ) {
                                                return;
                                            }
                                            const locale = a.locale ?? '_';
                                            const v = {
                                                id: `new-${uniqueId()}`,
                                                value: a.value,
                                            };
                    ...
                    <PrivacyField
                        ...
                        onChange={setPrivacy}
                        allowUnset
                        inheritedPrivacy={privacyInfo.data?.computedPrivacy}
                        disabled={
                            privacyInfo.data
                    ...
                    {isStory ? (
                        <div className="space-y-3 rounded-md border p-3">
                            <FormRow
                                label={t('upload.story_title', 'Story title')}
                            >
                                <Input
                                    value={storyTitle}
                                    onChange={e =>
                            ...
                            <FormRow
                                label={t('upload.story_tags', 'Story tags')}
                            >
                                <TagSelect
                                    multiple
                                    workspaceId={workspaceId}
                            ...
                            {workspaceId ? (
                                <AttributesEditor
                                    ...
                                    definitions={storyEditor.definitions}
                    ...
                    >
                        <AccordionItem value="attributes">
                            ...
                            <AccordionContent>
                                {workspaceId ? (
                                    <AttributesEditor
                                        attributes={assetEditor.attributes}
                                        definitions={assetEditor.definitions}
                            ...
                            <AccordionItem value="template">
                                ...
                                <AccordionContent>
                                    <SaveAsTemplateSection
                                        ...
                                        appliedTemplateId={templateIds[0]}
