...
export function WidgetOptionsDialog({
    ...
    return (
        <FormDialog
            ...
            title={t('cms.widget.options', 'Options')}
        ...
        >
            ...
            {type === 'searchGridWidget' ? (
                <>
                    <FormRow
                        label={t('saved_search.list.title', 'Saved searches')}
                    >
                        <SimpleSelect
                            value={form.savedSearchId ?? undefined}
                            onValueChange={v => set('savedSearchId', v)}
                            options={(savedSearches.data?.items ?? []).map(
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <FormRow label={t('cms.widget.max_items', 'Max items')}>
                            <Input
                                type="number"
                                value={form.maxItems ?? 20}
                                onChange={e =>
                                    set('maxItems', Number(e.target.value))
                                }
                            />
                        </FormRow>
                        <FormRow
                            label={t('display.thumb_size', 'Thumbnail size')}
                        >
                            <Input
                                type="number"
                                value={form.thumbSize ?? 200}
                                onChange={e =>
                                    set('thumbSize', Number(e.target.value))
                                }
                            />
                        </FormRow>
            ...
            {type === 'headerBarWidget' ? (
                <>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <FormRow
                            label={t('cms.widget.background', 'Background')}
                        >
                            <Input
                        ...
                        <FormRow label={t('cms.widget.color', 'Text color')}>
                            <Input
                                value={form.color ?? ''}
